...
class TestSaleOrder(TestSaleCommon):
    ...
    def setUpClass(cls):
        ...
        if "delivery_billing_mode" in cls.env["sale.order"]._fields:
            order_vals["delivery_billing_mode"] = "ppc"
        cls.sale_order = cls.env["sale.order"].with_user(cls.user).create(order_vals)
        so_form = Form(cls.sale_order)
        with so_form.order_line.new() as line:
            line.product_id = cls.product
            line.product_uom_qty = 2
        so_form.save()
        cls.sale_order.with_context(skip_tax_warning=True).action_confirm()
