...
class TestSaleOrder(TestSaleCommon):
    ...
    def test_invoice_wizard_does_not_set_so_responsible_as_invoice_follower(self):
        ...
        so.write({"client_order_ref": "Test PO Number", "delivery_billing_mode": "ppc"})
        so.with_context(skip_tax_warning=True).action_confirm()
        ...
        self.assertFalse(
            self.user
            in invoice.message_follower_ids.mapped("partner_id").mapped("user_ids")
        )
    ...
    def test_specific_user_as_invoice_follower(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "current_user_as_invoice_user.use_current_user", "False"
        )
        self.env["ir.config_parameter"].sudo().set_param(
            "current_user_as_invoice_user.specific_user_id", str(self.user.id)
        )
        ...
        followers_user_ids = invoice.message_follower_ids.mapped("partner_id").mapped(
            "user_ids"
        )
        self.assertEqual(len(followers_user_ids), 1, "Should have exactly one follower")
        self.assertEqual(
            followers_user_ids[0],
            self.user,
            "The only follower should be the specific user",
        )
    ...
    def test_current_user_as_invoice_follower(self):
        ...
        self.assertEqual(
            len(followers_user_ids),
            0,
            "No user should be automatically added as follower",
        )
    ...
    def test_current_user_as_invoice_user_and_follower(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "current_user_as_invoice_user.current_user_as_follower", "True"
        )
        ...
        for follower in invoice.message_follower_ids:
            _logger.info(
                f"Follower ID: {follower.id}, Partner: {follower.partner_id.name}, User IDs: {follower.partner_id.user_ids}"
            )
        ...
        _logger.info(
            f"After invalidation - Message followers count: {len(invoice.message_follower_ids)}"
        )
        ...
        _logger.info(
            f"Follower partners: {follower_partners.mapped('name')}, Count: {len(follower_partners)}"
        )
        _logger.info(
            f"Admin user ID: {admin.id}, Name: {admin.name}, Partner: {admin.partner_id.name}"
        )
        ...
        self.assertEqual(
            len(follower_partners), 1, "Un follower devrait être ajouté à la facture"
        )
        ...
        if admin_is_odoobott:
            self.assertEqual(
                follower_partners[0].name, "OdooBot", "Le follower devrait être OdooBot"
            )
        else:
            self.assertEqual(
                follower_partners[0],
                admin.partner_id,
                "Le follower devrait être le partenaire de l'utilisateur admin",
            )
